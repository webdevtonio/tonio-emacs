...
** Step 3
*** Clone the repo
=git clone https://github.com/webdevtonio/tonio-emacs ~/.emacs.d= 
** Step 4
*** Open emacs in terminal first
=emacs=
* Setup EXWM as new desktop
** Step 1
*** Open terminal and request root
=sudo su=
** Step 2
*** From home directory:
...
**** pulsemixer (not on aur)
- =https://github.com/GeorgeFilipkin/pulsemixer=.
- must install python-pip from aur
- =pip install --user pulsemixer (from home directory)
- user flag only installs for user, this avoids conflics with arch
**** Optional
- Qutebrowser (will try eventually, using firefox and chromium)
**** File manager
- Figure out why sunrise commander didn't load
- Consider trying treemacs
**** Composite manager
***** helps with screen tearing and fluidity
install =compton= from aur, =paranoid= helps to configure it. (need to read more about it)
  ...
  (use-package dashboard
    ...
    :config
      ...
      (setq dashboard-items '((recents  . 5)
                              (projects . 5)))
      (setq dashboard-banner-logo-title "Hello Antonio!"))
#+END_SRC
